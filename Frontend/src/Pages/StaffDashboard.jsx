...
import { 
    ...
    ExpandLess, ExpandMore, AccountCircle, Subject, Home, 
    Grading, Assessment, AccessTime, Campaign, Logout, 
    EventNote, AccessAlarm
...
const MetricCard = ({ title, value, icon: Icon, color, bgColor, loading }) => (
    ...
    >
        <CardContent>
            <Box sx={{ color, mb: 1.5, fontSize: 32 }}>
                <Icon fontSize="inherit" />
            </Box>
            <Typography variant="h4" sx={{ fontWeight: 700, color }}>
                {loading ? <CircularProgress size={24} sx={{ color }} /> : value}
            </Typography>
            <Typography variant="body2" sx={{ fontWeight: 500, color, opacity: 0.8, mt: 1 }}>
                {title}
            </Typography>
        </CardContent>
    </Card>
...
const StaffDashboard = () => {
    ...
    const fetchDashboardData = async () => {
        try {
            ...
            const sortedNotices = [...notices].sort((a, b) => new Date(b.issueDate) - new Date(a.issueDate));
    ...
    const currentDateText = now.toLocaleDateString('en-GB', {
        weekday: 'long',
        day: 'numeric',
        month: 'long',
        year: 'numeric'
    ...
    const menuItems = [
        ...
        { text: "PTM" , path:'/ptm'},
        { 
            text: "Assignment", icon: <Assignment />, onClick: () => setOpenAssignment(!openAssignment), open: openAssignment, 
            subItems: [
                { text: "Add Assignment", path: '/Assignment' },
        ...
        <Box sx={{ display: "flex", minHeight: "100vh", bgcolor: bgColor }}>
                    ...
                    {menuItems.map((item, index) => (
                        <React.Fragment key={index}>
                            <ListItem button onClick={item.path ? () => navigate(item.path) : item.onClick} 
                                sx={{ py: 1.5, transition: 'background-color 0.2s', '&:hover': { backgroundColor: lightBg }, ...(item.path === '/StaffDashboard' && location.pathname === '/StaffDashboard' && { backgroundColor: lightBg, color: primaryColor, fontWeight: 700 }) }}>
                                <ListItemIcon sx={{ minWidth: 45, color: location.pathname === item.path ? primaryColor : '#888' }}>{item.icon}</ListItemIcon>
                                <ListItemText primary={item.text} sx={{ '& .MuiListItemText-primary': { fontWeight: location.pathname === item.path ? 700 : 500, fontSize: '0.95rem', color: location.pathname === item.path ? primaryColor : '#555' } }} />
                                ...
                                <Collapse in={item.open} timeout="auto">
                                    <List disablePadding>
                                        {item.subItems.map((sub, i) => (
                                            <ListItem key={i} button onClick={() => navigate(sub.path)} sx={{ pl: 6, py: 1, '&:hover': { backgroundColor: '#f0f0f0' } }}>
                                                <ListItemText primary={sub.text} primaryTypographyProps={{ fontSize: '0.9rem', color: '#666' }} />
                                            </ListItem>
                                        ))}
                                    </List>
                                </Collapse>
            ...
            >
                <Grid container spacing={3}>
                                    ...
                                    {loading ? (
                                        ...
                                        recentNotices.map((notice) => (
                                            <Box key={notice._id} sx={{ p: 1.6, mb: 1.3, bgcolor: '#F8FAFF', borderRadius: '12px', border: '1px solid #E5EDFF' }}>
                                                <Typography sx={{ fontWeight: 700, color: darkText, fontSize: 14 }}>
                        ...
                        <Grid item xs={12}>
                            <Paper sx={{ p: 3.5, borderRadius: '22px', minHeight: 370, textAlign: 'center', display: 'flex', flexDirection: 'column', justifyContent: 'center', background: 'linear-gradient(160deg, #FFFFFF 0%, #F0FDFA 100%)', boxShadow: '0 10px 25px rgba(2, 6, 23, 0.08)' }}>
                                ...
                                <Typography variant="h3" sx={{ fontWeight: 900, color: darkText, mt: 0.5 }}>{currentTime}</Typography>
                                <Typography sx={{ color: greyText, fontWeight: 700, mt: 1 }}>{currentDateText}</Typography>
                                <Divider sx={{ my: 2.5 }} />
                                <Typography sx={{ fontWeight: 700, color: darkText }}>Auto refresh every 1 minute</Typography>
                                <Typography sx={{ fontSize: 12, color: greyText, mt: 0.3 }}>
                                    Dashboard widgets show live backend data
                                </Typography>
                            </Paper>
                        </Grid>
                </Grid>
